...
`global-ignore` allows to apply the `ignore` setting to all mutators & profiles registered. If `ignore` is specified for a given profile or mutator, it will be merged with `global-ignore`.
